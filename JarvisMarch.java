...
public class JarvisMarch extends ConvexHull
{
	...
	private PureStack<Point> leftChain = new ArrayBasedStack<Point>();
; 
	
	// right chain of the convex hull counterclockwise from highestPoint to lowestPoint
	private PureStack<Point> rightChain = new ArrayBasedStack<Point>(); 
	...
	public void constructHull()
	{
        ...
        Point highest = lowestPoint;

        for(int i = 1; i < pointsNoDuplicate.length; i++) {

            Point temp = pointsNoDuplicate[i];

            if(temp.getY() > highest.getY()) {
                highest = temp;
                highestIndex = i;
            }
        }
        highestPoint = pointsNoDuplicate[highestIndex];
		...
		int counter = (leftChain.size()+rightChain.size())-1;
		
		super.hullVertices = new Point[leftChain.size()+rightChain.size()];
	    
	    while(leftChain.size() > 0){
	    	
	    	super.hullVertices[counter] = leftChain.pop();
	    	counter = counter - 1;
	    }
	    while(rightChain.size() > 0){
	    	super.hullVertices[counter] = rightChain.pop();
	    	counter = counter - 1;
	    }
	...
	{
		// TODO 
        rightChain.push(highestPoint);
        
        Point nextPoint = nextVertex(highestPoint); 
        
        while(nextPoint.compareTo(lowestPoint) != 0){
        	
        	nextPoint = nextVertex(rightChain.peek());
        	
        	rightChain.push(nextPoint);
	...
	public void createLeftChain()
	{
        ...
        //leftChain.push(pointsNoDuplicate[highestIndex]);
        
        leftChain.push(lowestPoint);
        
        Point nextPoint = nextVertex(lowestPoint);

        
        while(nextPoint.compareTo(highestPoint) != 0){
        	
        	nextPoint = nextVertex(leftChain.peek());
        	
        	leftChain.push(nextPoint);
	...
	public Point nextVertex(Point v)
	{
		...
		PolarAngleComparator pComp = new PolarAngleComparator(v,false);
		
		int q = 0;
		for(int i = 0; i < pointsNoDuplicate.length;i++){
			
			if(pComp.compare(pointsNoDuplicate[i],pointsNoDuplicate[q]) < 0){
					q = i;
			}
			
		}
		return pointsNoDuplicate[q];
